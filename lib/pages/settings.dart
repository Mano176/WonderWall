...
class Settings extends StatefulWidget {
  ...
  final Function(List<String> allowedOrientations,
      [String? searchTerm, Random? random]) newWallpaper;
...
class _SettingsState extends State<Settings> {
  ...
  Widget buildGroup(
      ...
      required Function(List<Group> groups, [bool save]) setGroups}) {
    return Column(
          ...
          children: [
            ...
            Expanded(
              child: checkboxListTile(
                ...
                onHover: (value) {
                  if (widget.groups.any((e) => e.isEditing) ||
                      widget.groups
                          .any((e) => e.searchTerms.any((e) => e.isEditing))) {
                    return;
                  }
                ...
                setTitle: (value) {
                  ...
                  List<GroupElement> oldSearchTerms = group.searchTerms
                      .map((searchTerm) => GroupElement(searchTerm.title,
                          renameDateTime, true, searchTerm.enabled))
                      .toList();
                  Group oldGroup = Group(group.title, renameDateTime, true,
                      group.enabled, group.open, oldSearchTerms);
                  int index = widget.groups.indexOf(group);
                  widget.groups.insert(index, oldGroup);
                  Group? groupWithSameTitle = widget.groups
                      .where((group) => group.title == value)
                      .firstOrNull;
          ...
          child: SizedBox(
            ...
            child: Padding(
              ...
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  for (GroupElement element
                      in group.searchTerms.where((element) => !element.deleted))
                    checkboxListTile(
                      title: element.title,
                      titleSize: 15,
                      ...
                      onHover: (value) {
                        if (widget.groups.any((e) => e.isEditing) ||
                            widget.groups.any(
                                (e) => e.searchTerms.any((e) => e.isEditing))) {
                          return;
                        }
                      ...
                      setTitle: (value) {
                        DateTime renameDateTime = DateTime.now();
                        GroupElement oldSearchTerm = GroupElement(element.title,
                            renameDateTime, true, element.enabled);
                        int index = group.searchTerms.indexOf(element);
                        group.searchTerms.insert(index, oldSearchTerm);
                        group.searchTerms
                            .removeWhere((element) => element.title == value);
                        element.title = value;
                      ...
                      onSetAsWallpaper: () {
                        widget.newWallpaper(
                            widget.allowedOrientations.keys
                                .where((orientation) =>
                                    widget.allowedOrientations[orientation]!)
                                .toList(),
                            element.title,
                            null);
                      },
                  ...
                  Padding(
                    ...
                    child: OutlinedButton(
                      onPressed: () {
                        GroupElement element = GroupElement(
                            "New Search Term", DateTime.now(), false, true);
                  ...
                  child: SingleChildScrollView(
                    child: Column(
                        ...
                        children: [
                          ...
                          for (Group group
                              in widget.groups.where((group) => !group.deleted))
                            buildGroup(
                              group: group,
                          ...
                          Padding(
                            ...
                            child: OutlinedButton(
                              onPressed: () {
                                Group group = Group("New Group", DateTime.now(),
                                    false, true, true, []);
                          ...
                          onPressed: widget.wallpaperOnInterval
                              ? () => showTimePicker(
                                          ...
                                          initialTime: TimeOfDay(
                                              hour: widget.intervalHour,
                                              minute: widget.intervalMinute))
                                      .then((value) {
                                    if (value != null) {
                                      widget.setIntervalTime(
                                          value.hour, value.minute);
                                    }
                                  })
                              : null,
                          child: Text(
                              "New Wallpaper at ${widget.intervalHour.toString().padLeft(2, "0")}:${widget.intervalMinute.toString().padLeft(2, "0")}"),
                        ),
                      ),
                      const Text("Allowed orientations",
                          style: TextStyle(fontSize: 20)),
                      Row(
                        mainAxisAlignment: MainAxisAlignment.spaceEvenly,
                        children: widget.allowedOrientations.keys
                            .map((orientation) => Row(
                                  children: [
                                    Checkbox(
                                        value: widget
                                            .allowedOrientations[orientation],
                                        onChanged: widget.allowedOrientations[
                                                    orientation]! &&
                                                widget.allowedOrientations
                                                        .values
                                                        .where((value) => value)
                                                        .length ==
                                                    1
                              ...
                              child: Column(
                                ...
                                children: [
                                  Padding(
                                    ...
                                    child: OutlinedButton(
                                      onPressed:
                                          widget.credits["shareURL"] == null
                                              ? null
                                              : () => launchUrl(Uri.parse(
                                                  widget.credits["shareURL"]!)),
                                      child:
                                          const Text("Open image on Unsplash"),
                                    ),
                                  ),
                                  Padding(
                                    padding: const EdgeInsets.all(5.0),
                                    child: OutlinedButton(
                                      onPressed: widget.credits[
                                                  "photographer_url"] ==
                                              null
                                          ? null
                                          : () => launchUrl(Uri.parse(widget
                                              .credits["photographer_url"]!)),
                                      child: Text(
                                          "Photographer:\n${widget.credits["photographer_name"]}",
                                  ...
                                  Padding(
                                    ...
                                    child: OutlinedButton(
                                        onPressed: () =>
                                            widget.newWallpaperFromGroups(
                                                widget.groups),
                                        child: const Text("New Wallpaper")),
                                  ),
                                  Padding(
                                    padding: const EdgeInsets.all(5.0),
                                    child: OutlinedButton(
                                        onPressed: () => widget.newWallpaper(
                                            widget.allowedOrientations.keys
                                                .where((orientation) =>
                                                    widget.allowedOrientations[
                                                        orientation]!)
                                                .toList()),
                                        child:
                                            const Text("New Random Wallpaper")),
                                  ),
                                ],
                              ),
                            ),
            ...
            Align(
              ...
              child: Column(
                ...
                children: [
                  IconButton(
                    icon: () {
                      const Icon notLoggedInIcon = Icon(
